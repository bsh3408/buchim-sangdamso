...
export const db = {
  getOrCreateTeacher: (name, adminCode) =>
    rpc('get_or_create_teacher', { p_name: name, p_admin_code: adminCode }),
  ...
  bookSlot: (parentCode, slotId, studentName, className, parentPhone, topics, mode) =>
    rpc('book_slot', {
      p_parent_code: parentCode, p_slot_id: slotId, p_student_name: studentName,
      p_class_name: className, p_parent_phone: parentPhone, p_topics: topics, p_mode: mode,
    }),
  getBookingByToken: (cancelToken) =>
    rpc('get_booking_by_token', { p_cancel_token: cancelToken }),
  cancelBooking: (cancelToken) =>
    rpc('cancel_booking', { p_cancel_token: cancelToken }),
};
